...
public class Servlet06 extends GenericServlet {
    ...
    public void service(
        ...
        //
        // => URL 주소 창에 파라미터 값으로 한글을 입력했다 하더라고
        ...
        // 주의! 
        // => GET 요청할 때 URL에 한글이 포함될 경우 POST 처럼 따로 설정하지 않아도 된다.
...
//
// 7비트 장비?
// => 오래전에 구출된 일부 네트워크 방비(Gateway 등)가 7비트인 경우가 있다.
// => 이 경우 8비트 데이터를  통과시키면 전송하면 나올 때 1비트가 손실될 수 있다.
// 
// 해결책?
// => 8비트 값을 ASCII 문자로 바꿔 보내고,
// => 받은 쪽에서는 ASCII 문자를 원래의 바이트로 바꿔 사용한다.
// => 즉 "8비트 데이터를 ASCII 문자화시키는 방법" = "URL 인코딩"
//    "ASCII 문자화된 데이터를 뭔래의 바이트로 변환하는 것" = " "URL 디코딩"
// => 웹브라우저는 데이터르 보내기 전에 URL 인코딩한다.
//    원래 ASCII 문자는 그대로 두고 한글과 같은 7비트를 초과하는 코드인 경우
//    URL 인코딩을 진행한다.
// => 웹서버는 웹브라우저로붙 받은 URL 인코딩 값을 디코딩하여
//    원래의 바이트 값으로 자동으로 바꾼다.
//    따라서 개발자가 개입할 필요는 없다.
//
// 그런데 왜 우리가 URL 인코딩/디코딩을 알아야 하나요?
// => 웹브라우저와 웹서버 사이에 데이터를 주곳받는 HTTP 프로토콜을 분석하다 보면
...
//    URL 인코딩이 자동으로 처리되는 것이 아닌기 때문에
//    개발자가 java.net.URLEncoder 를 사용하여 직접 URL 인코딩 해야 한다.
//    => 웹브라우저가 보낸 데이터를 웹서버를 통해 받지 않고 직접 받는 경우가 있다.
//    이런 경우에도  개발자가 java.net.URLncoder를 사용하여
//    직접 소켓 프로그래밍으로 URL 디코딩 해야한다.
//    
