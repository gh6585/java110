...
public class Test15 {
    ...
    // => 프론트 컨트롤러가 데이터 타입을 바꿀 때 WebDateBinder를 통해 설정된
    //    도구를 사용한다.
    // => @InitBinder는 프론트 컨트롤러가 WebDateBinder를 사용하기 전에
    //    이 애노테이션이 지정된 멧드를 호출하여 먼저 초기화시칸 다음에
    //    사용할 것을 요구하는 애노테이션이다.
    ...
    public void initBinders(WebDataBinder binder) {
        ...
        // java.util.Date 객체로 바꿔주는 객체 도구
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        format.setLenient(false); // 24시간 표기 방법을 사용할 것인지 여부
        
        binder.registerCustomEditor(
                Date.class,
                //new DatePropertyEditor(format)); // 개발작자가 만든 것을 써도 되고
