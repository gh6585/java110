...
public class Test28 {
    ...
    // 테스트:
    //      http://localhost:8888/app/test28/m1
    //
    ...
    public String m5(HttpSession session) throws Exception {
        // 다른 페이지 컨트롤러의 @SessionAttribute에서
    ...
    public String logout2(SessionStatus status) throws Exception {
        ...
        // @SessionAttrubytes 에 지정된 값만 HttpSession에서 제거한다.
        ...
        return String.format("@SessionAttrubytes 관리 대상으 제거했다.\n");
...
//    특정 페이지 컨트롤러에서만 사용하고 관리될 값이라면 @SesionnAttributes로 등록하라!
//
// => 값 제거
//    HttpSession.invalidate()는 세션을 환전히 무효화시칸다. 
//    SessionStatus.setComplete()은 해당 페이지 컨트롤러의
//    @SessionAttrubutes에 지정된 값만 세션 객체에서 제거한다.
